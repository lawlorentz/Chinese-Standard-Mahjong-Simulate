...
class FeatureAgent(MahjongGBAgent):
    ...
    def __init__(self, seatWind):
        ...
        self.obs[self.OFFSET_OBS['SEAT_WIND']
                 ][self.OFFSET_TILE['F%d' % (self.seatWind + 1)]] = 1
        for i in range(36):  # 第143-146层为剩余的牌（没有摸过的牌）最开始没有发牌的时候我们全都置为1表示都有
            self.obs[144][i] = 1
            self.obs[145][i] = 1
            self.obs[146][i] = 1
            self.obs[143][i] = 1
    ...
    def request2obs(self, request):
        ...
        if t[0] == 'Wind':
            ...
            self.obs[self.OFFSET_OBS['PREVALENT_WIND']
                     ][self.OFFSET_TILE['F%d' % (self.prevalentWind + 1)]] = 1
            return
        ...
        if t[0] == 'Draw':
            ...
            for tile in set(self.hand):
                self.valid.append(
                    self.OFFSET_ACT['Play'] + self.OFFSET_TILE[tile])
                ...
                if not self.wallLast:
                    ...
                    if p == 3 and color in 'WTB':
                        ...
                        if tmp[0] in self.hand and tmp[1] in self.hand:
                            self.valid.append(
                                self.OFFSET_ACT['Chi'] + 'WTB'.index(color) * 21 + (num - 3) * 3 + 2)
                        if tmp[1] in self.hand and tmp[3] in self.hand:
                            self.valid.append(
                                self.OFFSET_ACT['Chi'] + 'WTB'.index(color) * 21 + (num - 2) * 3 + 1)
                        if tmp[3] in self.hand and tmp[4] in self.hand:
                            self.valid.append(
                                self.OFFSET_ACT['Chi'] + 'WTB'.index(color) * 21 + (num - 1) * 3)
        ...
        if t[2] == 'Peng':
            self.packs[p].append(
                ('PENG', self.curTile, (4 + p - self.tileFrom) % 4))
        ...
        if t[2] == 'Gang':
            self.packs[p].append(
                ('GANG', self.curTile, (4 + p - self.tileFrom) % 4))
        ...
        if t[2] == 'AnGang':
            ...
            if p == 0:
                ...
                self.obs[30][self.OFFSET_TILE[t[3]]] = 1  # 标记当前的暗杠
            else:
                self.isAboutKong = False
            return
        if t[2] == 'BuGang':
    ...
    '''
    dont't need to revise
    '''

    def _hand_embedding_update(self):
        # 这个函数仅用来每一步更改当前拥有的牌即2-5层，因此我们仅让2-5层初始化为0
        self.obs[self.OFFSET_OBS['HAND']: 6] = 0
        d = defaultdict(int)
        for tile in self.hand:
            d[tile] += 1
        for tile in d:
            self.obs[self.OFFSET_OBS['HAND']: self.OFFSET_OBS['HAND'] +
                     d[tile], self.OFFSET_TILE[tile]] = 1
